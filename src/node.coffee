...
class Node
  ...
  @attach: ( node ) -> attach node.parent, node.value
...
generic attach, ( Node.contains Style.Rule ), ( Type.isType Style.Rule ), ( node, style ) ->
...
generic attach, ( Node.contains Scope ), ( Type.isKind Scope ), ( node, scope ) ->
  attach node.parent, scope

# for media scope, compose the queries, and pass it up the tree
generic attach, ( Node.contains Media.Scope ), ( Type.isType Media.Scope ), ( node, scope ) ->
...
generic attach, ( Node.contains Layer.Scope ), ( Type.isType Layer.Scope ), ( node, layer ) ->
  node.value.append layer
